...
#define VALUES_PER_POINT 20
	...
	extern "C" __declspec(dllexport) int init(int n) {
		...
		GLuint posAttrib = glGetAttribLocation(shaderProgram, "mat_1");
		testError("GOGO 222");
		...
		for (int i = -size; i < size; ++i)
		{
			for (int j = -size; j < size; ++j)
			{
				...
				for (int k = 0, ch = 0; k < rand() % 4; ++k, ch += h)
				{
					...
					if (h < 0.05) {
						sphere(i, j, ch + h, 0.4 - (0.08*k), 0.0f, 0.0f, 0.0f);
						break;
					}
					if (k > 1 && ((float)rand() / (RAND_MAX))>0.2)
						cylinder(i, j, ch + h, 0.4 - (0.08*k), h, 1.0f, 1.0f, 1.0f, 0.0f, 1.0f, 0.0f, 0.0f);
					else
						box(i, j, ch + h, 0.4 - (0.08*k), 0.4 - (0.1*k), h, 1.0f, 1.0f, 1.0f, 0.0f, 1.0f, 0.0f, 0.0f);
				}
			}
